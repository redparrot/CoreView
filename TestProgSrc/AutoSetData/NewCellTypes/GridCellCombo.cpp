...
void CInPlaceList::endEditToSengMsg()
{
    HWND hWnd;
    hWnd = NULL;

    hWnd = ::FindWindow(NULL, "公式定义增加/编辑宏");

    if(hWnd != NULL)
    {
        ::PostMessage(hWnd, WM_SEND_TO_MODIFY_TEMPLATE, WPARAM(m_nRow), (LPARAM)m_nCol);
        return;
    }
    ...
    BOOL bFindWnd = FALSE;
    HWND pWind = NULL;
    pWind = ::FindWindow(NULL, "新建子宏");
    if (pWind != NULL)
    {
        bFindWnd = TRUE;
    }

    BOOL bFindWndNew = FALSE;
    HWND pWindNew = NULL;
    pWindNew = ::FindWindow(NULL, "编辑子宏");
    if (pWindNew != NULL)
    {
        bFindWndNew = TRUE;
    }
    ...
    if (bFindWnd == TRUE )
    {
        ::PostMessage(pWind, WM_SEND_TO_MODIFY_TEMPLATE, WPARAM(m_nRow), (LPARAM)m_nCol);
//		return ;
    }
    if(bFindWndNew == TRUE)
    {
        ::PostMessage(pWindNew, WM_SEND_TO_MODIFY_TEMPLATE, WPARAM(m_nRow), (LPARAM)m_nCol);
//		return ;
    }

    bFindWnd = FALSE;
    pWind = NULL;
    pWind = ::FindWindow(NULL, "编辑一级组合宏");
    if (pWind != NULL)
    {
        bFindWnd = TRUE;
    }
    ...
    bFindWndNew = FALSE;
    pWindNew = NULL;
    pWindNew = ::FindWindow(NULL, "新建一级组合宏");
    if (pWindNew != NULL)
    {
        bFindWndNew = TRUE;
    }
    ...
    {
        pWindNew = NULL;
        pWindNew = ::FindWindow(NULL, "新建二级组合宏");
        if (pWindNew != NULL)
        {
            bFindWndNew = TRUE;
        }
        ...
        {
            pWindNew = NULL;
            pWindNew = ::FindWindow(NULL, "新建三级组合宏");
            if (pWindNew != NULL)
            {
                bFindWndNew = TRUE;
            }
